...
def _validated_snapshot_plan(ctx: WebUIContext, metadata: dict[str, Any]):
    ...
    if mask_data_url:
        ...
        validate_edit_mask_data_urls(mask_data_url, image_data_urls[0])
        target_size = aligned_edit_mask_canvas_size(
            image_data_urls[0],
            requested_size=str(params.get("size") or ""),
            model=str(params.get("model") or snapshot.get("canonical_model_id") or "gpt-image-2"),
            max_edge=RESPONSES_EDIT_MASK_MAX_EDGE if profile.endswith("responses") else None,
        )
        image_data_urls[0], mask_data_url = normalize_edit_mask_data_urls(
            image_data_urls[0],
            mask_data_url,
            target_size=target_size,
        )
        params["size"] = f"{target_size[0]}x{target_size[1]}"
    ...
    requested_parameters = dict(snapshot.get("requested_parameters") or {})
    if mask_data_url and "canvas.size" in requested_parameters:
        requested_parameters["canvas.size"] = params["size"]
