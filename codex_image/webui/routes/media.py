...
_TASK_INPUT_FILENAME_RE = re.compile(
    rf"^(?P<task_id>{_TASK_ID_PATTERN})-(?:input|mask|guidance)-\d+-",
    re.IGNORECASE,
)
...
def _legacy_input_path(ctx: WebUIContext, filename: str) -> Path | None:
    ...
    editing_guidance = metadata.get("editing_guidance")
    if isinstance(editing_guidance, dict):
        for key in ("sharedBaseFile", "instructionMarksFile", "editRegionFile", "editMaskFile"):
            filename_value = editing_guidance.get(key)
            if isinstance(filename_value, str) and filename_value:
                allowed_names.add(filename_value)
