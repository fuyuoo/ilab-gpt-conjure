...
function resetSubmissionState(): void {
  ...
  Object.assign(methods, {
    ...
    updateEditRequestPreflight: async () => ({ issues: [] }),
    uploadInputs: () => [],
  });
...
test("pending preflight blocks duplicate submissions", async () => {
  resetSubmissionState();
  const pendingFetch = deferredFetch();
  let resolvePreflight!: (value: any) => void;
  methods.updateEditRequestPreflight = () => new Promise((resolve) => { resolvePreflight = resolve; });
  const first = methods.runTask();
  await methods.runTask();
  assert.equal(pendingFetch.calls(), 0);
  resolvePreflight({ issues: [] });
  await Promise.resolve();
  try {
    assert.equal(pendingFetch.calls(), 1);
  } finally {
...
test("preflight rejection and errors release the submission lock for retry", async () => {
  resetSubmissionState();
  const pendingFetch = deferredFetch();
  methods.updateEditRequestPreflight = async () => ({ issues: [{ level: "error" }] });
  await methods.runTask();
  assert.equal(pendingFetch.calls(), 0);
  methods.updateEditRequestPreflight = async () => { throw new Error("preflight failed"); };
  await assert.rejects(methods.runTask(), /preflight failed/);
  methods.updateEditRequestPreflight = async () => ({ issues: [] });
  const retry = methods.runTask();
  await Promise.resolve();
  try {
    assert.equal(pendingFetch.calls(), 1);
  } finally {
...
test("resize confirmation can resume submission after releasing the lock", async () => {
  resetSubmissionState();
  const pendingFetch = deferredFetch();
  let confirm!: () => Promise<void>;
  methods.updateEditRequestPreflight = async () => ({ issues: [{
    code: "responses_resize", level: "warning",
    values: { width: 3081, height: 1359, targetWidth: 2032, targetHeight: 896 },
  }] });
  methods.openConfirmPopover = (_anchor: any, options: any) => { confirm = options.onConfirm; };
  await methods.runTask();
  assert.equal(pendingFetch.calls(), 0);
...
test("edits during preflight remain an unsaved draft after submission", async () => {
  resetSubmissionState();
  const drafts = await import("../../codex_image/webui/frontend/src/composer-draft");
  ...
  const pendingFetch = deferredFetch();
  const submission = methods.runTask();
  prompt = "new draft during preflight";
  resolvePreflight({ issues: [] });
  await Promise.resolve();
  pendingFetch.resolveAll();
  await submission;
