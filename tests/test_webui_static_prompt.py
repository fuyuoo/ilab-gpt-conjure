...
class WebUIStaticPromptTests(WebUIStaticTestCase):
    ...
    def test_prompt_template_library_has_compact_entry_and_drawer(self) -> None:
        ...
        self.assertIn('class="ghost-button text-sm icon-text-button prompt-template-button"', prompt_html)
        ...
        self.assertRegex(styles, r"\.prompt-template-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)")
        self.assertRegex(styles, r"\.prompt-template-card\s*\{[^}]*border-radius:\s*8px")
        self.assertRegex(styles, r"\.prompt-template-card-thumb\s*\{[^}]*aspect-ratio:\s*4\s*/\s*3")
    ...
    def test_prompt_editor_paste_strips_web_html_styles(self) -> None:
        ...
        self.assertIn("range.insertNode(textNode)", source)
        self.assertIn("document.createTextNode(normalized)", source)
...
"""
    ...
    def test_preview_shows_prompt_popover_for_each_output(self) -> None:
        ...
        self.assertRegex(styles, r"\.prompt-action-row\s*\{[^}]*display:\s*flex")
        self.assertRegex(styles, r"\.prompt-action-row\s*\{[^}]*flex-wrap:\s*wrap")
        self.assertRegex(styles, r"\.prompt-action-row\s*\{[^}]*width:\s*auto")
        self.assertRegex(styles, r"\.prompt-action-row\s*\{[^}]*max-width:\s*min\(360px,\s*calc\(100% - 24px\)\)")
        self.assertRegex(styles, r"\.prompt-popover\s*\{[^}]*position:\s*fixed")
        self.assertRegex(styles, r"\.prompt-popover\s*\{[^}]*width:\s*min\(860px,\s*calc\(100vw - 24px\)\)")
    ...
    def test_prompt_popover_is_centered_on_anchor_and_clamped_to_viewport(self) -> None:
        script = self._frontend_script_source()
        styles = Path("codex_image/webui/static/styles.css").read_text(encoding="utf-8")

        self.assertIn("clampPopoverPosition", script)
        self.assertIn("anchor.getBoundingClientRect()", script)
        self.assertIn("anchorRect.left + anchorRect.width / 2 - width / 2", script)
        self.assertIn("window.innerWidth - width - margin", script)
        self.assertIn("window.innerHeight - height - margin", script)
        self.assertIn("Math.min(860, Math.max(360, window.innerWidth - margin * 2))", script)
        self.assertRegex(styles, r"\.prompt-popover\s*\{[^}]*width:\s*min\(860px,\s*calc\(100vw - 24px\)\)")
