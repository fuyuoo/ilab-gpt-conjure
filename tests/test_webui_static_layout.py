...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_image_editor_exposes_active_only_edit_region_workflow(self) -> None:
        ...
        output_controls_source = Path(
            "codex_image/webui/frontend/src/output-controls.ts"
        ).read_text(encoding="utf-8")
        image_strip_source = self._image_strip_source()

        self.assertIn('role="radiogroup"', html)
        self.assertIn('data-image-editor-guidance="instruction-marks"', html)
        self.assertIn('data-image-editor-guidance="edit-region"', html)
        self.assertIn('id="imageEditorMaskErase"', html)
        self.assertIn('id="imageEditorMaskRectangle"', html)
        self.assertIn('id="imageEditorMaskEllipse"', html)
        self.assertIn('id="imageEditorMaskRestore"', html)
        self.assertIn('id="imageEditorMaskClear"', html)
        ...
        self.assertIn("materializeEditMaskPixels", image_editor_source)
        self.assertIn("editMaskPreviewAlpha", image_editor_source)
        self.assertIn('guidance === "edit-region" ? "mask-erase" : "brush"', image_editor_source)
        self.assertIn('translate("imageEditor.emptyEditRegion")', image_editor_source)
        self.assertIn('form.append("mask", editMask)', task_submit_source)
        self.assertIn("await updateEditRequestPreflight(buildPreviewRequest())", task_submit_source)
        self.assertIn('issue.level === "error"', task_submit_source)
    ...
    def test_input_image_editor_javascript_contract_exists(self) -> None:
        ...
        self.assertNotIn("IMAGE_EDITOR_PROMPT_HINT_LEGACY", script)
        self.assertIn('translationsForKey("imageEditor.promptHint")', script)
        ...
        self.assertIn('"mask-erase", "mask-restore", "mask-rectangle", "mask-ellipse"].includes(tool)) return;', script)
        self.assertNotIn("function ensureImageEditorPromptHint()", script)
        self.assertIn("function promptForEditingGuidanceSubmission(", script)
        ...
        self.assertIn("instructionMarksAreSubmitted(", script)
        ...
        self.assertIn('imageEditorState.activeGuidance === "edit-region" ? "mask-erase" : "crop"', script)
    ...
    def test_input_image_editor_supports_layered_compositing(self) -> None:
        ...
        self.assertIn("function imageEditorCompositeCanvas(options:", image_editor_source)
