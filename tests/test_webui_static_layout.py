...
class WebUIStaticLayoutTests(WebUIStaticTestCase):
    ...
    def test_mask_input_is_removed_from_webui(self) -> None:
        ...
        self.assertIn('form.append("mask", editMask)', script)
    ...
    def test_input_image_editor_javascript_contract_exists(self) -> None:
        ...
        self.assertIn('if (!["select", "brush", "arrow", "crop", "fill", "eraser", "mask-erase", "mask-restore", "mask-rectangle", "mask-ellipse"].includes(tool)) return;', script)
        ...
        self.assertIn('imageEditorState.tool = imageEditorState.activeGuidance === "edit-region" ? "mask-erase" : "crop";', script)
    ...
    def test_input_image_editor_supports_layered_compositing(self) -> None:
        ...
        script = self._frontend_script_source()
        image_editor_source = self._image_editor_source()
        task_submit_source = self._task_submit_source()
        ...
        self.assertIn('editImageFiles.forEach((file) => form.append("reference_images", file))', task_submit_source)
        self.assertIn('editImageFiles.forEach((file) => form.append("images", file))', task_submit_source)
